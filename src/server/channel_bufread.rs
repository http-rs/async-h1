...
use async_std::io::{self, BufRead, Read, Seek, SeekFrom, Write};
...
type ChannelItem = io::Result<(Arc<Box<[u8]>>, usize)>;
type BufRecveiver = Receiver<ChannelItem>;
type BufSender = Sender<ChannelItem>;

lazy_static! {
    static ref EMPTY_BUF: Arc<Box<[u8]>> = Arc::new(vec![].into_boxed_slice());
}
...
impl<R: io::Read> BufReader<R> {
    ...
    /// # Ok(()) }) }
    /// ```
    pub fn new(inner: R) -> (BufReader<R>, BufSender) {
    ...
    /// # Ok(()) }) }
    /// ```
    pub fn with_capacity(capacity: usize, inner: R) -> (BufReader<R>, BufSender) {
        let (send, recv) = channel(0);
        
        let reader = BufReader {
            inner,
            recv: recv,
            buf: Arc::new(vec![0; capacity].into_boxed_slice()),
            pos: 0,
            cap: 0,
        };
...
impl<R> BufReader<R> {
    ...
    fn recv_buf(cx: &mut Context<'_>, recv: Pin<&mut BufRecveiver>, buf: &mut Arc<Box<[u8]>>) -> Poll<io::Result<usize>> {
        // drop the currently held buf so the io task can write to it
        // immediatly after we take the other
        *buf = EMPTY_BUF.clone();
        
        use async_std::stream::Stream;
        let new_buf = match futures_core::ready!(recv.poll_next(cx)) {
            Some(x) => x,
            None => {
                return Poll::Ready(
...
impl<R: Read> BufRead for BufReader<R> {
    fn poll_fill_buf<'a>(
        ...
        if *this.pos >= *this.cap {
            ...
            *this.cap = futures_core::ready!(Self::recv_buf(cx, this.recv, &mut this.buf))?;
            *this.pos = 0;
        }
...
impl<W: Write> Write for BufReader<W> {
    fn poll_write(
        self: Pin<&mut Self>,
        cx: &mut Context<'_>,
        buf: &[u8],
    ...
    fn poll_close(
        ...
        Write::poll_close(this.inner, cx)
    }
}
